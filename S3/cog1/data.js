...
define(["exports", "data", "glMatrix"], function(data, exports) {
	...
	function calcuateVertexNormalsFromPolygonNormals(){
		...
		// Initialize normal array.
		// Loop over polygons.
		for(var looping=0; looping< this.polygonVertices.length;looping++){
			var polygon = this.polygonVertices[looping];
			// Loop over vertices of polygon.
			for( var p= 0; p< polygon.length; p++){
				// Accumulate/add all polygon normals.
				if(polygon[p]<this.vertexNormals.length)
					vec3.add(this.vertexNormals[polygon[p]], this.polygonNormals[looping]);
