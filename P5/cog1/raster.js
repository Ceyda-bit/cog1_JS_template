 ...
 function (exports,shader,framebuffer,data) {
	...
	function assembleIntersectionForScanline(vertices, polygon, color, textureCoord, polygonTextureCoord, texture) {
		...
		var edgeStartTextureCoord, edgeEndTextureCoord = 0;
	  	...
	  	for (var i = polygon.length - 1; i > 0; i--) {
			var start_edge = vertices[polygon[i]];

			if(i == polygon.length - 1){
				lastDerivative = calcDerivative(start_edge[1], vertices[polygon[0]][1]);
			}else {
				lastDerivative = calcDerivative(start_edge[1], vertices[polygon[i - 1]][1]);
			}
	...
	function calcPlaneEquation(vertices, polygon, normal) {
		...
		AdivC =-A/C;
  		...
  		var dpolygon= vertices[polygon[0]];
		  D = -(dpolygon[0] *A + dpolygon[1] * B + dpolygon[2] * C);
		  inverseC = 1 / C;
